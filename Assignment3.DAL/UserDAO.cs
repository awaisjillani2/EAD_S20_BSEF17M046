    ...
    public static class UserDAO
    {
        public static UserDTO ValidateUser(String login, String password)
        {
            ...
            try
            {
                using (DBHelper helper = new DBHelper())
                {
                    ...
                    String query;
                    query = String.Format("Select ame from dbo.User where Login='{0}' and Password='{1}'", login, password);
        ...
        public static int SaveUser(UserDTO dto)
        {
            try
            {
                ...
                {
                    String query;
                    query = String.Format("Select count(*) from dbo.User where Login='{0}'", dto.login);
                    ...
                    {
                        query = String.Format("insert into dbo.User (Login, Name, Password) values('{0}','{1}','{2}')", dto.login, dto.name, dto.password);
